...
**Note:** The Workflow DevKit does not use `serve()`. Routes are auto-generated by `withWorkflow()` in next.config. The sentinel has no `.input()` or Zod schema (no arguments).

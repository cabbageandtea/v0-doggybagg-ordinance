...
9. **t._parse persists** – Add serverExternalPackages: [zod, workflow, @workflow/*] so packages are required not bundled; add webpack() in next.config; exclude /api/workflow from proxy. Workflow DevKit uses withWorkflow (no serve()). Sentinel has no .input() schema.
